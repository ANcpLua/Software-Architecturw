...
Design the architecture for a hospital management system based on a requirements class diagram. The architecture should consist of **4-6 building blocks** (subsystems), with each class assigned to exactly one building block.
...
Each class from the requirements diagram is assigned to exactly one subsystem, meeting the exercise constraint while maintaining architectural quality.
